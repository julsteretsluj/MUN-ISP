...
const THEME_GLYPHS = [Shield, HeartPulse, Landmark] as const;
...
export default function JourneyPage() {
  ...
  return (
    <>
      <PageIntro
        eyebrow="Page 2 · Compare & analyse"
        emoji="🌍"
        icon={<Route className="h-7 w-7" strokeWidth={1.75} />}
        title="My MUN journey"
      >
        <p>
      ...
      <Section
        ...
        title="Themes (not just a timeline)"
      >
        <p>
      ...
      <div className="space-y-10">
        {THEMES.map((theme, themeIndex) => {
          ...
          const ThemeGlyph = THEME_GLYPHS[themeIndex % THEME_GLYPHS.length];
          return (
            <section
              key={theme.name}
            ...
            >
              <div className="grid gap-6 lg:grid-cols-[240px_1fr] lg:gap-8">
                <aside className="lg:sticky lg:top-24 lg:self-start">
                  <div className="mb-3">
                    <span className="neu-chip grid h-12 w-12 place-items-center rounded-2xl text-[var(--neu-accent)] [&_svg]:h-6 [&_svg]:w-6">
                      <ThemeGlyph aria-hidden />
                    </span>
                  </div>
                  <div
                    className={`mb-3 inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-[11px] font-semibold uppercase tracking-wide ${accent.chip}`}
                  >
                    <span className="text-[13px] normal-case" aria-hidden>
                      {theme.emoji}
                    </span>
                    Theme {String(themeIndex + 1).padStart(2, "0")}
                  </div>
                  <h2 className="flex items-start gap-2.5 text-xl font-semibold tracking-tight text-[rgba(15,23,42,0.95)] sm:text-2xl">
                      ...
                      return (
                        ...
                        >
                          <div className="mb-2 flex items-start justify-between gap-3">
                            <p className="flex items-center gap-1.5 text-[12px] font-semibold uppercase tracking-wide text-[var(--neu-accent)]">
                              <MapPin
                                className="h-3.5 w-3.5 shrink-0 text-[rgba(79,70,229,0.85)]"
                                aria-hidden
                              />
                              {c.conference}
                            </p>
                          ...
                          <p className="flex items-start gap-2 text-[16px] font-semibold text-[rgba(15,23,42,0.95)]">
                            <Building2
                              className="mt-0.5 h-4 w-4 shrink-0 text-[rgba(100,116,139,0.55)]"
                              aria-hidden
                            />
                            <span>
                              {c.committee}{" "}
                          ...
                          <div className="mt-3 flex gap-2.5 rounded-xl bg-[rgba(15,23,42,0.04)] px-3 py-2">
                            <Target
                          ...
                          {agendaTopic ? (
                            <p className="mt-3 flex gap-2.5 rounded-xl border border-dashed border-[rgba(59,130,246,0.3)] bg-white/75 px-3 py-2 text-[13px] leading-relaxed text-[rgba(71,85,105,0.78)]">
                              <ScrollText
                          ...
                          <p className="mt-4 flex gap-2.5 border-t border-[rgba(148,163,184,0.2)] pt-4 text-[14px] leading-relaxed text-[rgba(100,116,139,0.65)]">
                            <MessageSquare
                              className="mt-0.5 h-4 w-4 shrink-0 text-[rgba(100,116,139,0.45)]"
                              aria-hidden
                            />
                            <span className="min-w-0">
      ...
      <Section
        emoji="🔗"
        icon={<GitCompare className="h-5 w-5" strokeWidth={1.75} />}
        title="How different approaches met the same underlying challenge"
      >
        <p>
